...
DEFINE_TEST(TestNested) {
  std::map<int, int> fib = {{0, 1}, {1, 1}, {2, 2}, {3, 3}, {4, 5}, {5, 8}};
  std::map<int, int> two = {{0, 1}, {1, 2}, {2, 4}};
  SingleTest(std::vector{std::map{std::pair{1,
                                            std::tuple{std::vector{fib, two},
                                                       std::set{fib, two},
                                                       std::string_view{"a long string"},
                                                       std::vector{&fib, &two}}}}});
}

DEFINE_TEST(TestSTLClass) {
  SingleTest(std::vector<std::complex<float>>{{1, 2}, {3, 4}, {5, 6}});
